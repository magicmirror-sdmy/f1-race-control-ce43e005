...
export const ServiceIndicator = ({ sensors, requiresService }: ServiceIndicatorProps) => {
      ...
      <PopoverTrigger asChild>
        <button
          className={`
            ...
            ${requiresService 
              ? 'border-warning bg-card cursor-pointer hover:bg-warning/20' 
        ...
        >
          ...
          {requiresService && (
            <span className="absolute inset-0 rounded-full border-2 border-warning animate-ping opacity-40" />
          )}
        </button>
      </PopoverTrigger>
