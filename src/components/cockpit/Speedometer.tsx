    ...
    <div className="flex flex-col items-center">
      <div className="relative w-[min(18vw,5rem)] aspect-square">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`text-sm sm:text-xl racing-number font-bold ${percentage > 80 ? 'text-destructive text-glow-red' : 'text-foreground'}`}>
            {Math.round(speed)}
          </span>
          <span className="text-[6px] sm:text-[8px] text-muted-foreground racing-text">KM/H</span>
        </div>
      </div>
      
      {/* Label */}
      <div className="text-[8px] sm:text-[10px] text-muted-foreground racing-text mt-0.5 sm:mt-1">SPEED</div>
    </div>
  );
};
